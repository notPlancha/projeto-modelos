...
\begin{itemize}
  ...
  \item \textbf{neighbourhood}: Embora este campo não inclua descrição no dicionário, nesta base de dados este campo descreve districtos de São Francisco, como \Sexpr{paste(head(unique(listings$neighbourhood), 2), collapse = " e ")}. %TODO check this https://tldrify.com/19ov
\end{itemize}
