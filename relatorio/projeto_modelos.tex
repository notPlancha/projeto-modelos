...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{df} \hlopt{%>%} \hlkwd{select}\hlstd{(name, price)} \hlopt{%>%} \hlkwd{arrange}\hlstd{(}\hlopt{-}\hlstd{price)} \hlopt{%>%} \hlkwd{head}\hlstd{(}\hlnum{7}\hlstd{)} \hlopt{%>%} \hlkwd{showT}\hlstd{(}\hlnum{TRUE}\hlstd{)}
\end{alltt}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{df} \hlopt{%>%} \hlkwd{ggplot}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{y}\hlstd{=price,} \hlkwc{x} \hlstd{= forcats}\hlopt{::}\hlkwd{fct_reorder}\hlstd{(neighbourhood, price,} \hlkwc{.fun}\hlstd{=median)))} \hlopt{+} \hlkwd{geom_boxplot}\hlstd{()} \hlopt{+} \hlkwd{coord_cartesian}\hlstd{(}\hlkwc{ylim}\hlstd{=}\hlkwd{c}\hlstd{(}\hlnum{0}\hlstd{, upper_limit))} \hlopt{+} \hlkwd{theme}\hlstd{(}\hlkwc{axis.text.x} \hlstd{=} \hlkwd{element_text}\hlstd{(}\hlkwc{angle} \hlstd{=} \hlnum{90}\hlstd{,} \hlkwc{vjust} \hlstd{=} \hlnum{0.5}\hlstd{,} \hlkwc{hjust}\hlstd{=}\hlnum{1}\hlstd{))} \hlopt{+} \hlkwd{labs}\hlstd{(}\hlkwc{x} \hlstd{=} \hlstr{""}\hlstd{)}
\end{alltt}
\end{kframe}\begin{figure}
...
\hlstd{df} \hlopt{%>%}
  ...
  \hlkwd{summarise}\hlstd{(}
    ...
    \hlkwc{sd} \hlstd{=} \hlkwd{sd}\hlstd{(price),}
    \hlkwc{min} \hlstd{=} \hlkwd{min}\hlstd{(price),}
    \hlkwc{max} \hlstd{=} \hlkwd{max}\hlstd{(price)}
  \hlstd{)} \hlopt{%>%} \hlkwd{showT}\hlstd{()}
\end{alltt}
\end{kframe}\begin{table}
\centering\begingroup\fontsize{10}{12}\selectfont
...
\begin{alltt}
\hlstd{df} \hlopt{%>%}
\hlkwd{select}\hlstd{(price, latitude, longitude,minimum_nights, number_of_reviews, calculated_host_listings_count, availability_365)} \hlopt{%>%}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{recipeUpSample} \hlkwb{<-} \hlkwd{recipe}\hlstd{(price} \hlopt{~} \hlstd{neighbourhood} \hlopt{+} \hlstd{latitude} \hlopt{+} \hlstd{longitude} \hlopt{+} \hlstd{room_type} \hlopt{+} \hlstd{minimum_nights} \hlopt{+} \hlstd{number_of_reviews} \hlopt{+} \hlstd{calculated_host_listings_count} \hlopt{+} \hlstd{availability_365,} \hlkwc{data}\hlstd{=df} \hlopt{%>%} \hlkwd{filter}\hlstd{(price} \hlopt{>} \hlnum{0}\hlstd{, neighbourhood} \hlopt{!=} \hlstr{"Golden Gate Park"}\hlstd{))} \hlopt{%>%}
  \hlkwd{step_upsample}\hlstd{(room_type,} \hlkwc{skip}\hlstd{=}\hlnum{FALSE}\hlstd{,} \hlkwc{over_ratio} \hlstd{=} \hlnum{0.363}\hlstd{)} \hlopt{%>%}
  \hlkwd{step_dummy}\hlstd{(neighbourhood, room_type)} \hlopt{%>%}
  \hlkwd{prep}\hlstd{()}
\hlstd{recipeNoUpSample} \hlkwb{<-} \hlkwd{recipe}\hlstd{(price} \hlopt{~} \hlstd{neighbourhood} \hlopt{+} \hlstd{latitude} \hlopt{+} \hlstd{longitude} \hlopt{+} \hlstd{room_type} \hlopt{+} \hlstd{minimum_nights} \hlopt{+} \hlstd{number_of_reviews} \hlopt{+} \hlstd{calculated_host_listings_count} \hlopt{+} \hlstd{availability_365,} \hlkwc{data}\hlstd{=df} \hlopt{%>%} \hlkwd{filter}\hlstd{(price} \hlopt{>} \hlnum{0}\hlstd{, neighbourhood} \hlopt{!=} \hlstr{"Golden Gate Park"}\hlstd{))} \hlopt{%>%}
\hlkwd{step_dummy}\hlstd{(neighbourhood, room_type)} \hlopt{%>%}
\hlkwd{prep}\hlstd{()}
\hlstd{dfAll} \hlkwb{<-} \hlkwd{bake}\hlstd{(recipeUpSample, df} \hlopt{%>%} \hlkwd{filter}\hlstd{(price} \hlopt{>} \hlnum{0}\hlstd{, neighbourhood} \hlopt{!=} \hlstr{"Golden Gate Park"}\hlstd{))}
\hlstd{dfAll} \hlopt{%>%} \hlkwd{group_by}\hlstd{(}\hlkwc{isHotel} \hlstd{= room_type_Hotel.room,} \hlkwc{isPrivateRoom} \hlstd{= room_type_Private.room,} \hlkwc{isSharedRoom} \hlstd{= room_type_Shared.room)} \hlopt{%>%} \hlkwd{summarise}\hlstd{(}\hlkwc{n}\hlstd{=}\hlkwd{n}\hlstd{(),} \hlkwc{freq}\hlstd{=n}\hlopt{/}\hlkwd{nrow}\hlstd{(dfAll),} \hlkwc{averagePrice} \hlstd{=} \hlkwd{mean}\hlstd{(price))} \hlopt{%>%} \hlkwd{showT}\hlstd{()}
\end{alltt}
\end{kframe}\begin{table}
\centering\begingroup\fontsize{10}{12}\selectfont
...
\hlkwd{testModel}\hlstd{(fit0, dfAll,} \hlstr{"fit0(price ~ .)"}\hlstd{)} \hlopt{%>%} \hlkwd{t}\hlstd{()} \hlopt{%>%} \hlkwd{showT}\hlstd{()}
\end{alltt}
\end{kframe}\begin{table}
\centering\begingroup\fontsize{10}{12}\selectfont
...
\hline
\cellcolor{gray!6}{R2} & \cellcolor{gray!6}{0.0481}\\
\hline
AIC & 1.62e+05\\
\hline
\cellcolor{gray!6}{MAPE} & \cellcolor{gray!6}{1.9}\\
\hline
rRMSE & 0.0395\\
\hline
\cellcolor{gray!6}{Breusch-Pagan} & \cellcolor{gray!6}{p = 4.05e-56}\\
\hline
...
\hline
\cellcolor{gray!6}{R2} & \cellcolor{gray!6}{0.0364} & \cellcolor{gray!6}{0.0388} & \cellcolor{gray!6}{0.126} & \cellcolor{gray!6}{0.362} & \cellcolor{gray!6}{0.509} & \cellcolor{gray!6}{0.569} & \cellcolor{gray!6}{0.576} & \cellcolor{gray!6}{0.536}\\
\hline
AIC & 5.42e+05 & 3.51e+05 & 7.03e+04 & 1.99e+04 & -4.42e+04 & -7.56e+04 & -1.41e+05 & -2.06e+05\\
\hline
\cellcolor{gray!6}{MAPE} & \cellcolor{gray!6}{133956} & \cellcolor{gray!6}{345} & \cellcolor{gray!6}{0.295} & \cellcolor{gray!6}{0.0908} & \cellcolor{gray!6}{0.307} & \cellcolor{gray!6}{1.59} & \cellcolor{gray!6}{212} & \cellcolor{gray!6}{62466}\\
\hline
rRMSE & 0.191 & 0.151 & 0.00689 & 0.00138 & 0.00284 & 0.00553 & 0.0123 & 0.0202\\
\hline
\cellcolor{gray!6}{Breusch-Pagan} & \cellcolor{gray!6}{p = 6.93e-45} & \cellcolor{gray!6}{p = 9.47e-49} & \cellcolor{gray!6}{p = 2.42e-66} & \cellcolor{gray!6}{p = 7.37e-115} & \cellcolor{gray!6}{p = 8.09e-230} & \cellcolor{gray!6}{p = 3.46e-323} & \cellcolor{gray!6}{p = 0} & \cellcolor{gray!6}{p = 0}\\
\hline
...
\end{knitrout}
Comparando as estatísticas das várias transformações do preço, podemos concluir que a transformação dos modelos $log(y)$ e $\sqrt{y^{-1}}$ são as mais apropriadas, a primeira com um valor de \textit{MAPE} e \text{rRMSE} mais baixo, e a segunda com um valor de $R^2$ mais alto. Nenhum desses modelos satifaz o pressuposto de homocedasticidade. Para continuará vai ser revisto os valores dos modelos em \textit{out-of-sample}. Para isso a base de dados foi dividida em 80\% para treino e 20\% para teste.\\
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{set.seed}\hlstd{(}\hlnum{123}\hlstd{)}
\hlstd{dfSplit} \hlkwb{<-} \hlkwd{initial_split}\hlstd{(df} \hlopt{%>%} \hlkwd{filter}\hlstd{(price} \hlopt{>} \hlnum{0}\hlstd{, neighbourhood} \hlopt{!=} \hlstr{"Golden Gate Park"}\hlstd{),} \hlkwc{prop} \hlstd{=} \hlnum{0.8}\hlstd{,} \hlkwc{strata} \hlstd{= room_type)}
\hlstd{dfTrain} \hlkwb{<-} \hlkwd{training}\hlstd{(dfSplit)} \hlopt{%>%} \hlkwd{bake}\hlstd{(recipeUpSample, .)}
\hlstd{dfTest} \hlkwb{<-} \hlkwd{testing}\hlstd{(dfSplit)} \hlopt{%>%} \hlkwd{bake}\hlstd{(recipeNoUpSample, .)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\hline
\cellcolor{gray!6}{R2} & \cellcolor{gray!6}{0.362} & \cellcolor{gray!6}{0.509} & \cellcolor{gray!6}{0.392} & \cellcolor{gray!6}{0.514}\\
\hline
AIC & 1.99e+04 & -4.42e+04 & 1.62e+04 & -3.83e+04\\
\hline
\cellcolor{gray!6}{MAPE} & \cellcolor{gray!6}{0.0908} & \cellcolor{gray!6}{0.307} & \cellcolor{gray!6}{0.093} & \cellcolor{gray!6}{0.289}\\
\hline
rRMSE & 0.00138 & 0.00284 & 0.00352 & 0.00764\\
\hline
\cellcolor{gray!6}{Breusch-Pagan} & \cellcolor{gray!6}{p = 7.37e-115} & \cellcolor{gray!6}{p = 8.09e-230} & \cellcolor{gray!6}{p = 1.6e-104} & \cellcolor{gray!6}{p = 1.44e-245}\\
\hline
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{fit1.noMN} \hlkwb{<-} \hlkwd{lm}\hlstd{(}\hlkwd{log}\hlstd{(price)} \hlopt{~} \hlstd{.} \hlopt{-} \hlstd{minimum_nights, dfAll)}
\hlstd{fit1} \hlkwb{<-} \hlkwd{lm}\hlstd{(}\hlkwd{log}\hlstd{(price)} \hlopt{~} \hlstd{., dfAll)}
\hlstd{fit1.MNsq} \hlkwb{<-} \hlkwd{lm}\hlstd{(}\hlkwd{log}\hlstd{(price)} \hlopt{~} \hlstd{., dfAll} \hlopt{%>%} \hlkwd{mutate}\hlstd{(}\hlkwc{mnsqd} \hlstd{= minimum_nights}\hlopt{^}\hlnum{2}\hlstd{,} \hlkwc{mncube} \hlstd{= minimum_nights}\hlopt{^}\hlnum{3}\hlstd{))}
\hlkwd{rbind}\hlstd{(}
  \hlkwd{testModel}\hlstd{(fit1.noMN, dfAll,} \hlstr{"noMN"}\hlstd{, log),}
  \hlkwd{testModel}\hlstd{(fit1, dfAll,} \hlstr{"withMN"}\hlstd{, log),}
  \hlkwd{testModel}\hlstd{(fit1.MNsq, dfAll} \hlopt{%>%} \hlkwd{mutate}\hlstd{(}\hlkwc{mnsqd} \hlstd{= minimum_nights}\hlopt{^}\hlnum{2}\hlstd{,} \hlkwc{mncube} \hlstd{= minimum_nights}\hlopt{^}\hlnum{3}\hlstd{),} \hlstr{"MNsq"}\hlstd{, log)}
\hlstd{)} \hlopt{%>%} \hlkwd{t}\hlstd{()} \hlopt{%>%} \hlkwd{showT}\hlstd{(F)}
\end{alltt}
\end{kframe}\begin{table}
\centering\begingroup\fontsize{10}{12}\selectfont
...
\hline
\cellcolor{gray!6}{R2} & \cellcolor{gray!6}{0.338} & \cellcolor{gray!6}{0.362} & \cellcolor{gray!6}{0.402}\\
\hline
AIC & 2.03e+04 & 1.99e+04 & 1.93e+04\\
\hline
\cellcolor{gray!6}{MAPE} & \cellcolor{gray!6}{0.0939} & \cellcolor{gray!6}{0.0908} & \cellcolor{gray!6}{0.0858}\\
\hline
rRMSE & 0.0014 & 0.00138 & 0.00133\\
\hline
\cellcolor{gray!6}{Breusch-Pagan} & \cellcolor{gray!6}{p = 1.6e-123} & \cellcolor{gray!6}{p = 7.37e-115} & \cellcolor{gray!6}{p = 3.1e-121}\\
\hline
...
\hlstd{fit} \hlopt{%>%} \hlkwd{tidy}\hlstd{()} \hlopt{%>%} \hlkwd{mutate}\hlstd{(}\hlkwc{estimate} \hlstd{=} \hlkwd{format}\hlstd{(estimate,} \hlkwc{scientific}\hlstd{=T),} \hlkwc{p.value} \hlstd{=} \hlkwd{format}\hlstd{(p.value,} \hlkwc{scientific}\hlstd{=T))} \hlopt{%>%} \hlkwd{showT}\hlstd{()}
\end{alltt}
\end{kframe}\begin{table}
\centering\begingroup\fontsize{10}{12}\selectfont
