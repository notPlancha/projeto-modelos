...
df %>% filter(price > 0) %>%
  ...
  mutate(
    ...
    isLuxuary = ifelse(price > upper_limit, 1, 0),
  ...
  ) %>% select(-neighbourhood, -room_type) %>% relocate(price, logPrice) %>% arrange(price) -> dfReady
dfReady
```
...
```{r}
# modelos
modelos <- data.frame(c("Desc", 'model', 'Call', 'RSquared', 'Residuals', 'Breusch–Pagan pvalue','autoplot'))
