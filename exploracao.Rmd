...
```{r}
df %>% ggplot(aes(x=price, y= neighbourhood, fill=neighbourhood)) + geom_density_ridges + theme_ridges() + theme(legend.position = "none")
```
...
```{r}
df2 <- df %>% select(latitude,longitude,price, minimum_nights, number_of_reviews, reviews_per_month, calculated_host_listings_count, availability_365, number_of_reviews_ltm)
