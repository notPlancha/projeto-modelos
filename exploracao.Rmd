...
df %>% ggplot(aes(x=price, y = forcats::fct_reorder(neighbourhood, -price, .fun=median))) + geom_boxplot() + xlim(0, upper_bound)
...
```{r}
df2 <- df %>% select(latitude, longitude, price, reviews_per_month, availability_365, number_of_reviews_ltm)
